...
//Methods to "eat" the current token
   ...
   public String eatCondOp() {
	  ...
	  if ((tok.ttype < '<' || tok.ttype > '>') && 
			  condOperatorTypes.contains(op)) {
		  return op;
	  }
	  
	  op += String.valueOf((char) tok.ttype);
	  
	  if (!condOperatorTypes.contains(op)) {
    ...
    * Otherwise, moves to the next token.
    *
    * @return 0 for hash, 1 for btree
    */
   public int eatIndexType() {
      if (!matchKeyword("hash") && !matchKeyword("btree"))
         throw new BadSyntaxException();
      int idxtype = matchKeyword("hash") ? 0 : 1;
      nextToken();
      return idxtype;
   }
   ...
   private void initCondOperatorTypes() {
      condOperatorTypes = Arrays.asList("<", "<=", "=", "<>", "!=", ">", ">=");
   }
}